...
class SumGradientAcquisitionFunction(AnalyticAcquisitionFunction):
    '''
    '''
    def __init__(self,obj_fn_gp,best_f,eps_values, model: Model,itr, maximize: bool = False) -> None:
        ''' 
            We use the AcquisitionFunction constructor, since that of 
            AnalyticAcquisitionFunction performs some validity checks 
            that we don't want here.
    ...
    def temp_schedule(self,num = 25):
        r = np.log(2)/num
        # r = 0
        # print(np.exp(-r*self.iter))
        return np.exp(-r*self.iter)
