...
def optimize_acq_func_and_get_candidates(acq_func, grad_acq,obj_fn_gp,best_f,eps_values, bounds, grad_gps,
    ...
    if order:
        ...
        if grad_acq_name == "PIGradientAcquisitionFunction":
            acq_func = grad_acq(obj_fn_gp,best_f,eps_values,grad_gps)
            candidate, _ = optimize_acqf(
                acq_function=acq_func,
                bounds=bounds,
                q=1,
