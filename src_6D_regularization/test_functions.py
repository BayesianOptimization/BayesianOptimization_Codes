...
class IllustrationND(ObjectiveFunction):
    ...
    def trn_loss(self, x, l):
        alpha = torch.tensor([10, 20, 30, 40, 50, 60])
        return torch.sum((x-alpha)**2, dim=1) + torch.sum(l*(x**2), dim=1)

    def val_loss(self, x):
        alpha = torch.tensor([.5, 1.5, 2.5, 3.5, 4.5, 5.5])
        return torch.sum((x-alpha)**2)
    ...
    def train_loss(self, x, l):
        ...
        alpha = torch.tensor([10, 20, 30, 40, 50, 60])
        trn_loss = torch.sum((x-alpha)**2, dim=1) + torch.sum(l*(x**2), dim=1)
